...
# 1. Four-Way Split Function
def four_way_split(X, y, val1_size=0.2, val2_size=0.1, test_size=0.2):
    """Split data into train, validation (val1), ensemble-validation (val2), and test sets."""
    total_len = len(X)
    test_len = int(total_len * test_size)
    val2_len = int(total_len * val2_size)
    val1_len = int(total_len * val1_size)

    train_idx = range(0, total_len - test_len - val2_len - val1_len)
    val1_idx = range(total_len - test_len - val2_len - val1_len, total_len - test_len - val2_len)
    val2_idx = range(total_len - test_len - val2_len, total_len - test_len)
    test_idx = range(total_len - test_len, total_len)

    return (
        X.iloc[train_idx], X.iloc[val1_idx], X.iloc[val2_idx], X.iloc[test_idx],
        y.iloc[train_idx], y.iloc[val1_idx], y.iloc[val2_idx], y.iloc[test_idx]
    )
...
# Split data into train, val1, val2, and test
X_train, X_val1, X_val2, X_test, y_train, y_val1, y_val2, y_test = four_way_split(X_cont, y_cont)
...
rf_model, _ = optimize_model_hyperparameters(RandomForestWrapper, rf_param_grid, X_train, y_train, validation_data=(X_val1, y_val1))
...
xgb_model, _ = optimize_model_hyperparameters(XGBoostWrapper, xgb_param_grid, X_train, y_train, validation_data=(X_val1, y_val1))
...
lasso_meta_model_stacking, _ = optimize_model_hyperparameters(
    LassoWrapper, meta_param_grid, X_train=stacking_features_train, y_train=y_train, validation_data=(stacking_features_val2, y_val2)
)
