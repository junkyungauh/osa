...
def preprocess_data_with_general_features(file_path, target_column, lag_steps=None, rolling_window=None, ema_window=None, normalize=True):
    ...
    # Signal Features (only retain rolling features, drop frequency-like interactions)
    signal_cols = [col for col in data.columns if col.startswith('sig')]
    if signal_cols:  # Ensure signal_cols is not empty
        signal_df = data[signal_cols]
    
        # Rolling statistics for signals
        for col in signal_cols:
            feature_data[f'{col}_roll_mean'] = signal_df[col].rolling(window=rolling_window, closed='right').mean()
            feature_data[f'{col}_roll_std'] = signal_df[col].rolling(window=rolling_window, closed='right').std()
            feature_data[f'{col}_roll_sum'] = signal_df[col].rolling(window=rolling_window, closed='right').sum()
    else:
        print("No signal columns found in the dataset.")
    ...
    if rolling_window:
        ...
        feature_data[f'{target_column}_roll_max_{rolling_window}'] = (
            data[target_column].rolling(window=rolling_window, closed='right').max()
        )
        feature_data[f'{target_column}_roll_min_{rolling_window}'] = (
            data[target_column].rolling(window=rolling_window, closed='right').min()
        )

    # Momentum and Rate-of-Change (ROC)
    feature_data[f'{target_column}_roc'] = data[target_column].pct_change()
    feature_data[f'{target_column}_momentum'] = data[target_column] - data[target_column].shift(rolling_window)
    ...
    # Cumulative Sum
    feature_data[f'{target_column}_cumsum'] = data[target_column].cumsum()
    ...
    # Drop NaNs introduced by lagging and rolling
